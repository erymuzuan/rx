define([
    ...
    var isBusy = ko.observable(false), activate = function (routeData) {
    ...
    var showMap = function () {
        ...
        if (!buildingId) {
            mapvm.geocode(vm.building.Address.Street() + "," + vm.building.Address.City() + "," + vm.building.Address.Postcode() + "," + vm.building.Address.State() + "," + "Malaysia.").then(function (result) {
                if (result.status) {
                    mapvm.init({
                        panel: 'map',
                        draw: true,
        ...
        $.when(pathTask, centerTask).then(function (path, center) {
            isBusy(false);
            if (center[0]) {
    ...
    }, buildingPolygon = null, polygoncomplete = function (shape) {
        ...
        datacontext.post(data, "/Building/SaveMap").then(function (e) {
