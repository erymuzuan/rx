...
bespoke.sph.domain.FixedLengthTextFormatterPartial=function(a){const b=require("durandal/system"),c=ko.observableArray(["$record"]),d=function(i){i&&0>c().indexOf(i)&&c.push(i)},e=ko.computed(function(){return ko.unwrap(a.SampleStoreId)&&ko.unwrap(a.Delimiter)}),g=function(i,j,l){if(l!==j){const p=i[j],q=l<j?-1:1;for(let r=j;r!==l;r+=q)i[r]=i[r+q];i[l]=p}},h=function(i,j){const l=a.FieldMappingCollection,m=ko.unwrap(l),n=m.indexOf(i);g(m,n,n+j),l(m)};return{isWizardOk:e,addDetailsRow:function(){const i=new bespoke.sph.domain.FlatFileDetailTag(b.guid());i.nameChangedSubscription=i.Name.subscribe(d),this.DetailRowCollection.push(i)},removeDetailsRow:function(i){var j=this;return function(){j.DetailRowCollection.remove(i),i.nameChangedSubscription&&i.nameChangedSubscription.dispose()}},parentOptions:c,moveUp:function(i){h(i,-1)},moveDown:function(i){h(i,1)},addField:function(){const i=new bespoke.sph.domain.FixedLengthTextFieldMapping(b.guid());this.FieldMappingCollection.push(i)},removeField:function(i){var j=this;return function(){j.FieldMappingCollection.remove(i)}}}};
bespoke.sph.domain.FormDialogPartial=function(a){var b=require("durandal/system"),c=function(){var f=ko.unwrap(a.DialogButtonCollection);_(f).each(function(i){i.canMoveDown(!0),i.canMoveUp(!0)});var g=_(f).first(),h=_(f).last();g&&g.canMoveUp(!1),h&&h.canMoveDown(!1)},d=function(f,g,h){if(h!==g){var i=f[g],j=h<g?-1:1;for(var l=g;l!==h;l+=j)f[l]=f[l+j];f[h]=i}},e=function(f,g){var h=a.DialogButtonCollection().indexOf(f),i=ko.unwrap(a.DialogButtonCollection);d(i,h,h+g),_(i).each(function(j){j.canMoveDown(!0),j.canMoveUp(!0)}),_(i).first().canMoveUp(!1),_(i).last().canMoveDown(!1),a.DialogButtonCollection(i)};return c(),{removeDialogButton:function(f){a.DialogButtonCollection.remove(f),c()},addDialogButton:function(){a.DialogButtonCollection.push(new bespoke.sph.domain.DialogButton({WebId:b.guid(),canMoveDown:!1})),c()},moveDown:function(f){e(f,1)},moveUp:function(f){e(f,-1)}}},bespoke.sph.domain.DialogButtonPartial=function(a){var b=ko.observable(!0),c=ko.observable(!0);return{canMoveDown:c,canMoveUp:b}};
