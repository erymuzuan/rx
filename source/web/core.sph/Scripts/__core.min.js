...
return String._toFormattedString(!1,arguments)},$type._toFormattedString=function(c,d){for(var e='',f=d[0],g=0;;){// Find the next opening or closing brace
...
ko.bindingHandlers.tree={init:function(a,b){// let member = null;
let f=null,g=null,h=null,j=null;const k=require(objectbuilders.system),l=b(),m=ko.unwrap(l.entity),o=ko.unwrap(l.searchbox),r=l.selected,t={text:m.Name(),state:{opened:!0,selected:!0}},u=function(B){var C=ko.unwrap(B.TypeName)||ko.unwrap(B.$type);return 0>C.indexOf(",")&&(C=ko.unwrap(B.$type)),C},w=function(B){B.children=_(B.data.MemberCollection()).map(function(C){const D=u(C);return{text:C.Name(),state:"open",type:D,data:C}}),_(B.children).each(w)},x=function(B,C){ko.isObservable(C.childOfValueMember)||(C.childOfValueMember=ko.observable(!0));var D=$(a).jstree(!0),E=_(B.parents).map(function(G){return D.get_node(G)}),F=_(E).find(function(G){return"Bespoke.Sph.Domain.ValueObjectMember, domain.sph"===G.type});C.childOfValueMember(F||!1)},y=function(B){const C=$(a).jstree(!0),D=C.get_node(B),E=D.data;E&&($(`#${B} a.jstree-anchor>i.fa-question`).remove(),$(`#${B} a.jstree-anchor>i.fa-certificate`).remove(),ko.isObservable(E.IsNullable)&&ko.unwrap(E.IsNullable)&&$(`#${B} a.jstree-anchor>i.jstree-icon`).after("<i class=\"fa fa-question\" title=\"Nullable\" style=\"margin-right:5px;color:orange\"></i>"),ko.unwrap(E.AllowMultiple)&&$(`#${B} a.jstree-anchor>i.jstree-icon`).after("<i class=\"fa fa-certificate\" title=\"Allow multiple\" style=\"margin-right:5px;color:green\"></i>"))};(function(){t.children=_(m.MemberCollection()).map(function(B){return{text:ko.unwrap(B.Name),state:"open",type:u(B),data:B}}),_(t.children).each(w),$(a).on("select_node.jstree",function(B,C){if("function"==typeof r){h&&(h.dispose(),h=null),g&&(g.dispose(),g=null),f&&(f.dispose(),f=null),j&&(j.dispose(),j=null);const D=C.node.data;if(D){if("default"===C.node.type)return;ko.isObservable(D.childOfValueMember)||(D.childOfValueMember=ko.observable(!0)),ko.isObservable(D.allowFilter)||(D.allowFilter=ko.observable(!0));var E=$(a).jstree(!0),F=_(C.node.parents).map(function(H){return E.get_node(H)}),G=_(F).find(function(H){return"Bespoke.Sph.Domain.ValueObjectMember, domain.sph"===H.type});// we also need to disable for collection member
if(D.childOfValueMember(G||!1),!G){const H=_(F).find(function(I){const J=I.data;return"Bespoke.Sph.Domain.ComplexMember, domain.sph"===I.type&&ko.unwrap(J.AllowMultiple)});H&&D.allowFilter(!1)}D.childOfValueMember()||E.edit(C.node),r(D),f=r().Name.subscribe(function(H){$(a).jstree(!0).rename_node(C.node,H),console.log(`rename ${H}`)}),g=r().AllowMultiple.subscribe(H=>y(C.node.id)),ko.isObservable(r().IsNullable)&&(g=r().IsNullable.subscribe(H=>y(C.node.id))),"function"==typeof r().TypeName&&(j=r().TypeName.subscribe(function(H){$(a).jstree(!0).set_type(C.node,H)}))}}}).on("move_node.jstree",function(B,C){var D=$(a).jstree(!0),E=C.node.data,F=D.get_node(C.old_parent),G=F.data.MemberCollection||m.MemberCollection,H=D.get_node(C.parent),I=H.data.MemberCollection||m.MemberCollection;G.remove(function(J){return ko.unwrap(E.WebId)===ko.unwrap(J.WebId)}),I.splice(C.position,0,E),H.children.forEach(y),F.children.forEach(y)}).on("rename_node.jstree",function(B,C){const D=C.node.data;D.Name(C.text)}).on("open_node.jstree",function(B,C){y(C.node.id),C.node.children.forEach(y)}).jstree({core:{animation:0,check_callback:function(B,C,D,E,F){if("move_node"===B){const G=C.data,H=$(a).jstree(!0),I=H.get_node(D);return!!G&&!!I&&(ko.isObservable(G.childOfValueMember)||x(C,G),!ko.unwrap(G.childOfValueMember)&&"Bespoke.Sph.Domain.ValueObjectMember, domain.sph"!==I.type)}return!0},themes:{stripes:!0},data:t},dnd:{is_draggable:function(B){var C=B.data;return C&&ko.unwrap(C.childOfValueMember)?!1:!0}},contextmenu:{items:function(B){var C=$(a).jstree(!0),D=_(B.parents).map(function(L){return C.get_node(L)}),E=_(D).find(function(L){return"Bespoke.Sph.Domain.ValueObjectMember, domain.sph"===L.type}),F=C.get_selected();if(E)return[];var G={label:"Add Simple Child",action:function(){const L=new bespoke.sph.domain.SimpleMember({WebId:k.guid(),TypeName:"System.String, mscorlib",Name:"Member_Name"}),M=$(a).jstree("get_selected",!0),N=M[0].data;return!!F.length&&(F=F[0],F=C.create_node(F,{state:"open",type:"System.String, mscorlib",text:"Member_Name",data:L}),!!F&&(C.edit(F),N&&N.MemberCollection?N.MemberCollection.push(L):m.MemberCollection.push(L),!0))}},H={label:"Add Value Object Child",action:function(){var L=new bespoke.sph.domain.ValueObjectMember({WebId:k.guid(),TypeName:"Bespoke.Sph.Domain.ValueObjectDefinition, domain.sph",Name:"Member_Name"}),M=$(a).jstree("get_selected",!0),N=M[0].data;return!!F.length&&(F=F[0],F=C.create_node(F,{state:"open",type:"Bespoke.Sph.Domain.ValueObjectMember, domain.sph",text:"Member_Name",data:L}),!!F&&(C.edit(F),N&&N.MemberCollection?N.MemberCollection.push(L):m.MemberCollection.push(L),L.TypeName.subscribe(function(){// TODO : clear the node children and populate the node according to the value object definition members
}),!0))}},I={label:"Add Complex Child",action:function(){var L=new bespoke.sph.domain.ComplexMember({WebId:k.guid(),Name:"Member_Name"}),M=$(a).jstree("get_selected",!0),N=M[0].data;return!!F.length&&(F=F[0],F=C.create_node(F,{state:"open",type:"Bespoke.Sph.Domain.ComplexMember, domain.sph",text:"Member_Name",data:L}),!!F&&(C.edit(F),N&&N.MemberCollection?N.MemberCollection.push(L):m.MemberCollection.push(L),!0))}},J={label:"Remove",action:function(){// now delete the member
var L=C.get_selected(!0)[0],M=C.get_node($("#"+L.parent)),N=M.data;if(N&&"function"==typeof N.MemberCollection){var O=_(N.MemberCollection()).find(function(Q){return Q.WebId()===L.data.WebId()});N.MemberCollection.remove(O)}else{var P=_(m.MemberCollection()).find(function(Q){return Q.WebId()===L.data.WebId()});m.MemberCollection.remove(P)}return!!F.length&&(C.delete_node(F),!0)}},K=[];return("default"===B.type||"Bespoke.Sph.Domain.ComplexMember, domain.sph"===B.type)&&(K.push(G),K.push(H),K.push(I)),console.log(B),"default"!==B.type&&K.push(J),K}},types:{default:{icon:"fa fa-clipboard"},"System.String, mscorlib":{icon:"fa fa-text-width",valid_children:[]},"System.DateTime, mscorlib":{icon:"glyphicon glyphicon-calendar",valid_children:[]},"System.Int32, mscorlib":{icon:"fa fa-sort-numeric-asc",valid_children:[]},"System.Decimal, mscorlib":{icon:"glyphicon glyphicon-usd",valid_children:[]},"System.Boolean, mscorlib":{icon:"glyphicon glyphicon-ok",valid_children:[]},"Bespoke.Sph.Domain.ValueObjectMember, domain.sph":{icon:"fa fa-object-ungroup"},"Bespoke.Sph.Domain.ComplexMember, domain.sph":{icon:"fa fa-object-group"}},plugins:["contextmenu","dnd","types","search"]})})();const z=$(a).jstree(!0);$.each($(a).find("li"),function(B,C){y(C.id)});var A=!1;$(o).keyup(function(){A&&clearTimeout(A),A=setTimeout(function(){const B=$(o).val();z.search(B)},250)})}},ko.bindingHandlers.help={init:function(a,b){var f=$(a),g=ko.unwrap(b());f.click(function(h){h.preventDefault(),window.open("/docs/#"+g)})}};var substringMatcher=function(a){return function(f,g){var h,j;// an array that will be populated with substring matches
...
(function(){Function.prototype.partial=function(){var a=this,b=Array.prototype.slice.call(arguments);return function(){for(var c=0,d=0;d<b.length&&c<arguments.length;d++)void 0===b[d]&&(b[d]=arguments[c++]);return a.apply(this,b)}}})();
