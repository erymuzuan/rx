...
function(context, logger, router, system, validation, eximp, dialog, watcher, config, app) {
    ...
    var entity = ko.observable(new bespoke.DevV1_product.domain.Product(system.guid())),
        ...
        activate = function(entityId) {
            ...
            context.loadOneAsync("EntityForm", "Route eq 'new-product'")
                .then(function(f) {
                form(f);
                return watcher.getIsWatchingAsync("Product", entityId);
            })
                .then(function(w) {
                watching(w);
                return $.getJSON("i18n/" + config.lang + "/new-product");
            })
                .then(function(n) {
                i18n = n[0];
                ...
                return context.get("/api/products/" + entityId);
        ...
        defaultCommand = function() {
            ...
            context.post(data, "/api/products/", headers)
        ...
        attached = function(view) {
            // validation
            validation.init($('#new-product-form'), form());
        ...
        saveCommand = function() {
            return defaultCommand()
                .then(function(result) {
                if (result.success) {
                    return app.showMessage("Your product has been added", ["OK"]);
                } else {
                    return Task.fromResult(false);
                }
            })
