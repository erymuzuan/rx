    ...
    public partial class ScheduledTriggerActivity : Activity
    {
        public override BuildValidationResult ValidateBuild(WorkflowDefinition wd)
        {
            ...
            if(string.IsNullOrWhiteSpace(this.NextActivityWebId))
                result.Errors.Add(new BuildDiagnostic(this.WebId,$"[ScheduledTriggerActivity] ->{this.Name} is missing NextActivityWebId"));

            if(this.IntervalScheduleCollection.Count == 0)
                result.Errors.Add(new BuildDiagnostic(this.WebId,$"[ScheduledTriggerActivity] -> {this.Name} is missing triggers"));
