...
namespace Bespoke.Sph.Tests.CosmosDb
{
    ...
    public class QueryEndpointTest
    {
        ...
        public void CompileQueryFieldsAndFilter()
        {
            ...
            var json = new Patient().CompileToCosmosDbSql(dsl);
        ...
        {
            Assert.NotEmpty(performer);
            Assert.NotEmpty(performerValues);
            var query = new QueryEndpoint
            {
                Name = "Patients Born in 60s",
                Route = "~/api/patients/born-in-60s",
            ...
            var json = default(Patient).CompileToCosmosDbSql(query.QueryDsl);
            var jo = JObject.Parse(json);
