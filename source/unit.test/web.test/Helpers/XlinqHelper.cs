...
namespace Bespoke.Sph.WebTests.Helpers
{
    public static class XLinqHelper
    {
        ...
        public const string XML_NAMESPACE = "http://www.bespoke.com.my/";
        ...
        public static string RemoveDomainName(this string threadUserName)
        {
            if (string.IsNullOrEmpty(threadUserName))
                throw new ArgumentNullException(nameof(threadUserName));
            if (threadUserName.Contains("\\"))
            {
                return threadUserName.Split('\\').Last();
            }

            if (threadUserName.Contains("@"))
            {
                return threadUserName.Split('@').First();
            }
        ...
        private static XmlSerializer GetDefaultSerializer(Type type)
        {
            ...
            {
                var serializer = new XmlSerializer(type, XML_NAMESPACE);
                try
                {
                ...
                {
                    return new XmlSerializer(type, XML_NAMESPACE);
                }
            }
