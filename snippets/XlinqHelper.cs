...
namespace ToolsHelper
{
    public static class XLinqHelper
    {
        ...
        {
            int int32Va;
            if(int.TryParse(value, out int32Va))
                return int32Va;
        ...
        public static string SetAttributeStringValue(this XElement xml, string value, params string[] elements)
        {
            ...
            for (int i = 0; i < elements.Length - 1; i++)
            {
                ...
                if (null == ele)
                    throw new ArgumentException("Cannot find the element" + elements[i],"elements");
            }
            var att = ele.Attribute(elements.Last());
            if(att ==  null)
            {
                ele.SetAttributeValue(elements.Last(), value);
            }
            else
            {
                att.Value = value;
        ...
        public static string SetElementStringValue(this XElement xml, string value, params string[] elements)
        {
            XNamespace x = XmlNamespace;
            XElement ele = xml;
            foreach (var s in elements)
