...
{
   [RoutePrefix("api/dbo/address")]
   public partial class AddressController : ApiController
   {

       [Route("{addressId:int}")]
       [HttpDelete]
       public async Task<HttpResponseMessage> Remove(int addressId)
       {

            var context = new AddressAdapter();
            await context.DeleteAsync(addressId);
       ...
       public async Task<object> Get(int addressId)
       {
            ...
            var context = new AddressAdapter();
            var item  =await context.LoadOneAsync(addressId);
       ...
       [Route("")]
       [HttpPost]
       public async Task<HttpResponseMessage> Insert([FromBody]Address item)
       {
       ...
       public async Task<object> List(string filter = null, int page = 1, int size = 40, bool includeTotal = false, string orderby = null)
       {
           ...
           if (size > 200)
            ...
            var translator = new OdataSqlTranslator<Address>(null,"Address" ){Schema = "dbo"};
            var sql = translator.Select(string.IsNullOrWhiteSpace(filter) ? "AddressId gt 0" : filter, orderby);
            var count = 0;
            ...
            {
                var translator2 = new OdataSqlTranslator<Address>(null, "Address"){Schema = "dbo"};
                ...
                if (count >= lo.ItemCollection.Count())
                    nextPageToken = string.Format(
                        "/api/dbo/address/?filter={0}&includeTotal=true&page={1}&size={2}", filter, page + 1, size);
            }

            string previousPageToken = string.Format("/api/dbo/address/?filter={0}&includeTotal=true&page={1}&size={2}", filter, page - 1, size);
            if(page == 1)
       ...
       [Route]
       [HttpPut]
       public async Task<HttpResponseMessage> Save([FromBody]Address item)
       {
